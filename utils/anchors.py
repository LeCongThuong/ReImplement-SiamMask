...
class Anchors:
    ...
    def __init__(self, cfg):
        ...
        self.anchor_num = len(self.scales) * len(self.ratios) * len(self.anchor_density**2)
        self.anchors = None
        self.all_anchors = None
        self.gernerate_anchors()
    ...
    def generate_anchors(self):
        ...
        for x_offsets, y_offsets in zip(x_offsets.flatten(), y_offsets.flatten()):
            for r in self.ratios:
                ...
                for s in self.scales:
                    w = ws * s
                    h = ws * s
                    self.anchors[count][:] = [-w * 0.5 + x_offsets, -h * 0.5 + y_offsets, w * 0.5 + x_offsets, h * 0.5 + y_offsets][:]
                    count += 1
